...
var Router = class extends EventTarget {
  ...
  render() {
    var _a, _b;
    log2(`Rendering route ${this.context.url.pathname}${this.context.url.search}`, { context: this.context, route: this.route });
    return (_b = (_a = this.route) == null ? void 0 : _a.render) == null ? void 0 : _b.call(_a, this.context);
  }
  /**
   * @param {URL} url 
   * @returns {Route | null}
  ...
  _matchRoute(url) {
    ...
    log2(`No route matched for ${url.pathname}${url.search}`, url);
    return null;
  }
  _notifyUrlChanged() {
  ...
  async navigate(url, options = {}) {
    ...
    log2(`Navigating to ${url.pathname}${url.search}`, { context: this.context, route: this.route });
    const plugins = [
      ...((_a = this.config) == null ? void 0 : _a.plugins) ?? [],
      ...((_b = this.route) == null ? void 0 : _b.plugins) ?? []
    ];
    for (const plugin of plugins) {
      try {
        const result = await ((_c = plugin == null ? void 0 : plugin.shouldNavigate) == null ? void 0 : _c.call(plugin, this.context));
        if (result) {
    ...
    for (const plugin of plugins) {
      try {
        await ((_d = plugin == null ? void 0 : plugin.beforeNavigation) == null ? void 0 : _d.call(plugin, this.context));
      } catch (e) {
    ...
    for (const plugin of plugins) {
      try {
        await ((_e = plugin == null ? void 0 : plugin.afterNavigation) == null ? void 0 : _e.call(plugin, this.context));
      } catch (e) {
